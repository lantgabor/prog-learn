...
def draw_fireworks(firework_list, projectile_list):
    ...
    for i in range(len(firework_list)):
        if firework_list[i][3] < counter and firework_list[i][2] < firework_list[i][1]:
            pygame.draw.rect(
                screen,
                firework_list[i][4],
                [firework_list[i][0], firework_list[i][1], 10, 10],
                0,
                3,
            )
            firework_list[i][1] -= 7
    ...
    for i in range(len(projectile_list)):
        color = (
            projectile_list[i][4][0],
            projectile_list[i][4][1],
            projectile_list[i][4][2],
            projectile_list[i][5] * 4,
        )
        pygame.draw.circle(
            surface, color, (projectile_list[i][0], projectile_list[i][1]), 3
        )
        ...
        if (
            projectile_list[i][5] < 0
            or WIDTH < projectile_list[i][0] < -3
            or HEIGHT < projectile_list[i][0]
        ):
            remove.append(i)
