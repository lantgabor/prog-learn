...
class Pawn(Piece):
    ...
    def get_valid_moves(self, board):
        ...
        # Move forward
        if (
            0 <= self.row + direction < 8
            and board[self.row + direction][self.col] is None
        ):
            ...
            if (
                self.row == start_row
                and board[self.row + 2 * direction][self.col] is None
            ):
        ...
        for dc in [-1, 1]:
            ...
            if 0 <= new_row < 8 and 0 <= new_col < 8:
                if (
                    ...
                    and board[new_row][new_col].color != self.color
                ):
                    moves.append((new_row, new_col))
        ...
        for dr, dc in knight_moves:
            ...
            if 0 <= new_row < 8 and 0 <= new_col < 8:
                if (
                    ...
                    or board[new_row][new_col].color != self.color
                ):
                    moves.append((new_row, new_col))
        return moves
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if (
                    board[row][col]
                    and board[row][col].color == current_turn
                    and not selected_piece
                ):
        ...
        if not game_over:
            ...
            turn_text = turn_font.render(
                f"{current_turn.capitalize()}'s turn", True, (255, 0, 0)
            )
            ...
            winner_text = game_over_font.render(
                f"{winner.capitalize()} Wins!", True, (255, 215, 0)
            )
